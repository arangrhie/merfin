...
## Use nls to fit 1 peak model (haploid case)
###############################################################################

nls_2peak<-function(x, y, k, estKmercov, estLength, max_iterations){
  model2 = NULL
  
  if (VERBOSE) { cat("trying nls_1peak standard algorithm\n") }
  
  try(model2 <- nls(y ~ (((1-d)*((1-r)^k))  * dnbinom(x, size = kmercov * 2 / bias, mu = kmercov * 2) * length + 
                        (d*(1-r)^(2*k))    * dnbinom(x, size = kmercov * 4 / bias, mu = kmercov * 4) * length), 
  ...
  if(class(model2) == "try-error"){
    if (VERBOSE) { cat("retrying nls_1peak with port algorithm\n") }
...
estimate_Genome_4peak2<-function(kmer_hist_orig, x, y, k, readlength, round, foldername, haploid){
...
report_results<-function(kmer_hist,kmer_hist_orig, k, container, foldername, haploid)
{
  ...
  for (i in 1:(akcov*5-1)){
    ...
    p.values<-c(na.zero(error_kmers[i]/total_kmers/totalP),as.numeric(fitted_hist[i,])/totalP)
    
    plot_table<-rbind(plot_table,p.values)
...
while (round < NUM_ROUNDS)
{
  ...
  model_4peaks <-
    estimate_Genome_4peak2(kmer_prof, x, y, k, readlength, round, foldername, haploid)
